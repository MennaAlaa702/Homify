...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun validateForm(): Boolean {
        ...
        if (firstName.isEmpty())  { showError(etFirstName, tvErrorFirstName, getString(R.string.err_first_name_req)); isValid = false }
        if (lastName.isEmpty())   { showError(etLastName,  tvErrorLastName,  getString(R.string.err_last_name_req));  isValid = false }
        if (email.isEmpty() || !Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            showError(etEmail, tvErrorEmail, getString(R.string.err_email_invalid)); isValid = false
        }
        if (nationalId.isEmpty()) { showError(etNationalId, tvErrorNationalId, getString(R.string.err_nid_req)); isValid = false }
        else if (nationalId.length != 14) { showError(etNationalId, tvErrorNationalId, getString(R.string.err_nid_length)); isValid = false }
        ...
        if (phone.isEmpty()) { tvErrorPhone.text = getString(R.string.err_role_req); tvErrorPhone.visibility = View.VISIBLE; isValid = false }
        else if (phone.length != 11) { tvErrorPhone.text = "Phone must be 11 digits"; tvErrorPhone.visibility = View.VISIBLE; isValid = false }
        else { tvErrorPhone.visibility = View.GONE }
        if (password.isEmpty()) { showError(etPassword, tvErrorPassword, getString(R.string.err_pass_req)); isValid = false }
        if (confirmPassword.isEmpty()) { showError(etConfirmPassword, tvErrorConfirmPassword, getString(R.string.err_confirm_pass_req)); isValid = false }
        else if (password != confirmPassword) { showError(etConfirmPassword, tvErrorConfirmPassword, getString(R.string.err_pass_match)); isValid = false }
        if (!cbTerms.isChecked) { Toast.makeText(this, "You must agree to the Terms of Service", Toast.LENGTH_SHORT).show(); isValid = false }
