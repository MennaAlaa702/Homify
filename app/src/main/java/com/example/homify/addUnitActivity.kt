...
class addUnitActivity : AppCompatActivity() {
    ...
    private val pickImagesLauncher = registerForActivityResult(
        ...
        if (uris.isNotEmpty()) {
            ...
            findViewById<TextView>(R.id.tv_upload_desc)
                .text = getString(R.string.image_s_selected, uris.size)
        }
    }
    ...
    private fun setupSpinner() {
        val types = arrayOf(
            getString(R.string.select_unit_type),
    ...
    private fun validateForm(): Boolean {
        ...
        // التعديل الجديد: التحقق من أن اللينك مش فاضي وإنه لينك حقيقي
        if (etLocation.text.isEmpty()) {
            showError(etLocation, tvErrLocation, reqMsg)
            isValid = false
        } else if (!Patterns.WEB_URL.matcher(etLocation.text).matches()) {
            showError(etLocation, tvErrLocation, "Please enter a valid URL (e.g., https://maps...)")
            isValid = false
        }
